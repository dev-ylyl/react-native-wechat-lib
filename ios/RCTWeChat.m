...
- (NSData *)compressImage:(UIImage *)image toByte:(NSUInteger)maxLength {
  ...
  while (data.length > maxLength && data.length != lastDataLength) {
    ...
    CGSize size = CGSizeMake(
        ...
        (NSUInteger)(resultImage.size.height *
...
RCT_EXPORT_METHOD(registerApp
  ...
  self.appId = appid;
  callback(@[ [WXApi registerApp:appid universalLink:universalLink]
                  ? [NSNull null]
                  : INVOKE_FAILED ]);
}
...
//                   :(RCTResponseSenderBlock)callback)
// {
//     callback(@[[WXApi registerApp:appid withDescription:appdesc] ? [NSNull
//     null] : INVOKE_FAILED]);
// }
...
RCT_EXPORT_METHOD(openCustomService : (NSString *)corpid : (NSString *)url) {
...
RCT_EXPORT_METHOD(shareImage
  ...
  if (imageUrl == NULL || [imageUrl isEqual:@""]) {
    callback([NSArray
        arrayWithObject:@"shareImage: The value of ImageUrl cannot be empty."]);
    return;
  }
  ...
  if (range.length == 0) {
    callback([NSArray
        arrayWithObject:
            @"shareImage: ImageUrl value, Could not find file suffix."]);
    return;
  }
...
RCT_EXPORT_METHOD(shareLocalImage
  ...
  if (imageUrl == NULL || [imageUrl isEqual:@""]) {
    callback([NSArray
        arrayWithObject:
            @"shareLocalImage: The value of ImageUrl cannot be empty."]);
    return;
  }
  NSRange range = [imageUrl rangeOfString:@"."];
  if (range.length == 0) {
    callback([NSArray
        arrayWithObject:
            @"shareLocalImage: ImageUrl value, Could not find file suffix."]);
    return;
  }
...
RCT_EXPORT_METHOD(launchMiniProgram
                  : (NSDictionary *)data
                  : (RCTResponseSenderBlock)callback) {
  WXLaunchMiniProgramReq *launchMiniProgramReq =
  ...
  launchMiniProgramReq.miniProgramType =
      [data[@"miniProgramType"] integerValue];
  void (^completion)(BOOL);
  completion = ^(BOOL success) {
    callback(@[ success ? [NSNull null] : INVOKE_FAILED ]);
    return;
...
- (void)onReq:(BaseReq *)req {
  if ([req isKindOfClass:[LaunchFromWXReq class]]) {
    ...
    [self.bridge.eventDispatcher sendDeviceEventWithName:RCTWXEventNameWeChatReq
                                                    body:body];
  }
}
...
- (void)onResp:(BaseResp *)resp {
  if ([resp isKindOfClass:[SendMessageToWXResp class]]) {
    ...
    [self.bridge.eventDispatcher sendDeviceEventWithName:RCTWXEventName
                                                    body:body];
    ...
    for (WXCardItem *cardItem in r.cardAry) {
      NSMutableDictionary *item = @{
        ...
        @"encryptCode" : cardItem.encryptCode,
        @"appId" : cardItem.appID
      }
                                      .mutableCopy;
      [arr addObject:item];
    }
  ...
  else if ([resp isKindOfClass:[WXOpenCustomerServiceResp class]])
   {
        ...
        NSMutableDictionary *body = @{@"errCode":@(res.errCode)}.mutableCopy;
        body[@"errStr"] = res.extMsg;
        body[@"type"] = @"WXOpenCustomerServiceResp.Resp";
